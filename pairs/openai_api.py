...
class OpenAIChatModel:
    ...
    def compare(self, prompts, max_workers=4):
        result_list = self.multi_threading_openai_chat_completion(
            prompts, self.single_call_compare, max_workers=max_workers
        )
    ...
    def rate_score(self, prompts, max_workers=4):
        result_list = self.multi_threading_openai_chat_completion(
            prompts, self.single_call_rate_score, max_workers=max_workers
        )
        ...
        try:
            if "davinci" in api_params["engine"]:
                ...
                response = client.completions.create(
                    ...
                    logprobs=(
                        api_params["top_logprobs"] if api_params["logprobs"] else None
                    ),
                    echo=True,
                )
            else:
    ...
    if "instruct" in api_params["engine"]:
        ...
        for idx, token in enumerate(response.choices[0].logprobs.tokens):
            if token in ["A", "B"]:
                ...
                prob_A = (
                    np.exp(token_prob_candidate["A"])
                    if "A" in token_prob_candidate
                    else 0
                )
                prob_B = (
                    np.exp(token_prob_candidate["B"])
                    if "B" in token_prob_candidate
                    else 0
                )
                break
    else:  # for chat model
