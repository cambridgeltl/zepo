...
from .utils import (
    CompareResultObject,
    insert_index_to_anchors,
    get_calibration_shift,
    calculate_entropy,
)
...
def moving_average(sum, val, idx):
    scale = min(idx, 5)
    return sum * scale / (scale + 1) + val / (scale + 1)
    return sum * idx / min(idx + 1, 3) + val / min(idx + 1, 3)
...
def is_better_than_prob(id1, id2, preference_matrix, permutate=False):
    if permutate:
        return (preference_matrix[id1][id2] + 1 - preference_matrix[id2][id1]) / 2
    else:
        return preference_matrix[id1][id2]


def merge_sort_indices(preference_matrix, params, permutate=False):
    ...
    indices = list(range(len(preference_matrix)))
    random.shuffle(indices)
    indices = merge_sort(
        indices, 0, len(indices), preference_matrix, params, permutate=permutate
    )
    return indices


def merge_sort(indices, left, right, preference_matrix, params, permutate=False):
    if right - left > 1:
        mid = (left + right) // 2
        merge_sort(indices, left, mid, preference_matrix, params, permutate=permutate)
        merge_sort(indices, mid, right, preference_matrix, params, permutate=permutate)
        if params["confidence_beam"]:
            merge_with_confidence_beam(
                indices,
                left,
                mid,
                right,
...
def merge(indices, left, mid, right, preference_matrix, params, permutate=False):
    ...
    while i < len(left_copy) and j < len(right_copy):
        ...
        compare_result = is_better_than_prob(
            left_copy[i],
            right_copy[j],
        ...
        if compare_result > 0.5:  # ['prob_A']>compare_result['prob_B']:
...
def merge_with_confidence_beam(
    indices, left, mid, right, preference_matrix, params, permutate=False
):
    def get_probA(i, j):
    ...
    prob_A_matrix = np.zeros(
        (len(left_copy), len(right_copy))
    )  # prob_A_matrix[i, j] is the probability of A better than B
    ...
    if prob_A_matrix[0, 0] > 0.5 + prob_gap:
        beam = [
            BeamItem(
                index_pathway=[("B", 0)],
                cum_prob=np.log(prob_A_matrix[0, 0] + 1e-9) * coef,
                pointer_B=0,
            ),
        ]
        ...
        beam = [
            BeamItem(
                index_pathway=[("A", 0)],
                cum_prob=np.log(1 - prob_A_matrix[0, 0] + 1e-9) * coef,
                pointer_A=0,
            ),
        ]
    else:
    ...
    # sort beam according to cum_prob
    beam.sort(key=lambda x: x.cum_prob, reverse=True)
    beam = beam[:beam_size]
    ...
    for i in range(len(left_copy) + len(right_copy) - 1):
        ...
        for beam_item in beam:
            for choice in ["A", "B"]:
                ...
                if (
                    beam_item_copy.pointer_A < len(left_copy) - 1
                    and beam_item_copy.pointer_B < len(right_copy) - 1
                    ...
                    if (choice == "A" and prob_A > 0.5 + prob_gap) or (
                        choice == "B" and 1 - prob_A > 0.5 + prob_gap
                    ):
                        continue
                    # beam_item_copy.cum_prob *= 1-prob_A if choice == 'A' else prob_A
                    logprob = (
                        np.log(1 - prob_A + 1e-9)
                        if choice == "A"
                        else np.log(prob_A + 1e-9)
                    )
                    beam_item_copy.cum_prob = moving_average(
                        beam_item_copy.cum_prob, logprob * coef, i + 1
                    )
                ...
                current_step = (
                    choice,
                    (
                        beam_item_copy.pointer_A
                        if choice == "A"
                        else beam_item_copy.pointer_B
                    ),
                )
...
def binary_search_insert_index(input, output, params, anchors_idx, target_idx):
    ...
    while left <= right:
        ...
        if "progress_bar" in params:
            params["progress_bar"].update(1)
        compare_result = is_better_than_prob(
            anchors_idx[mid], target_idx, input, output, params=params
        )
...
def merge_sort_with_scale(input, output, scores, params, sort_size=100):
    ...
    sorted_anchor_indices = merge_sort_indices(
        input[:sort_size], output[:sort_size], params
    )
    ...
    for idx in range(sort_size, len(input)):
        insert_index = binary_search_insert_index(
            input, output, params, sorted_anchor_indices, idx
        )
    ...
    sorted_full_indices = insert_index_to_anchors(
        sorted_anchor_indices, searech_result, sort_size
    )
...
if __name__ == "__main__":
    ...
    input, output, scores = (
        input[: args.eval_size],
        output[: args.eval_size],
        scores[: args.eval_size],
    )
    ...
    if params["confidence_beam"]:
        ...
        params["progress_bar"] = tqdm(
            total=int(len(input) * np.log2(len(input))), desc="Processing"
        )
    ...
    if args.scaling_anchor_size == 0:
        ...
        ranking_indices = merge_sort_with_scale(
            input, output, scores, params, sort_size=args.scaling_anchor_size
        )
    ...
    calculate_correlation(
        predicted_score=human_scores[ranking_indices], reference_score=modified_scores
    )
    ...
    if args.save_path is not None:
        results = {
            ...
            "compare_log": {
                str(key): val for key, val in params["compare_log"].items()
            },
        }
