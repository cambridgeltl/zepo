...
def load_hanna(file_path):
    ...
    relevance = [
        round(np.mean(dp["relevance"]), 1) for dp in list(processed_df.values())
    ]
    coherence = [
        round(np.mean(dp["coherence"]), 1) for dp in list(processed_df.values())
    ]
    ...
    engagement = [
        round(np.mean(dp["engagement"]), 1) for dp in list(processed_df.values())
    ]
    complexity = [
        round(np.mean(dp["complexity"]), 1) for dp in list(processed_df.values())
    ]
    scores = {
...
def correlation_analysis(results):
    ...
    spearman_original, _, _ = calculate_correlation(
        results["human_scores"], results["pred_scores"]
    )
    print("------------------")
    ...
    spearman_geval, _, _ = calculate_correlation(
        results["human_scores"], weighted_gpt_scores
    )
    print("------------------")
